...
def create_caltech_101_disk_folder(
    ...
    ignore = ['BACKGROUND_Google', 'Faces_easy']
    rename = {
        'airplanes': 'airplane',
        'Faces': 'face',
        'Leopards': 'leopard',
    ...
    labels = [label for label in labels if label not in ignore]
    for label in tqdm(labels):
        ...
        train_output_path = os.path.join(output_path, "train", label if label not in rename else rename[label])
        test_output_path = os.path.join(output_path, "test", label if label not in rename else rename[label])
