...
from MetaCLIP.open_clip.factory import create_model_and_transforms
from MetaCLIP.open_clip.transform import get_mean_std
from MetaCLIP.open_clip.model import CLIP, VisualTransformer, Transformer, ResidualAttentionBlock
from MetaCLIP.training.data import get_data
from MetaCLIP.training.distributed import is_master, init_distributed_device, world_info_from_env
from MetaCLIP.training.logger import setup_logging
from MetaCLIP.training.scheduler import cosine_lr
from MetaCLIP.training import train
from MetaCLIP.training.checkpoint import load_checkpoint, unwrap_model
...
def main(args):
    ...
    if hasattr(args, "cap_model"):
        from MetaCLIP.training.train_altogether import create_captioner
    ...
    if 'train' not in data or hasattr(args, "eval") and args.eval:  # huxu: merge native/SLIP eval.
        ...
        from MetaCLIP.training.slip_evaluate import slip_evaluate
        from MetaCLIP.open_clip.tokenizer import tokenize
    ...
    if hasattr(args, "engine"):
        ...
        for model_code in os.listdir(f"src/training"):
            if model_code.startswith("train"):
                module = importlib.import_module("MetaCLIPtraining." + model_code[:-len(".py")])
    ...
    if hasattr(args, "eval") and args.eval and any(v in data for v in ('val', 'imagenet-val', 'imagenet-v2')):
        from MetaCLIP.training.slip_evaluate import slip_evaluate
        from MetaCLIP.open_clip import tokenize
