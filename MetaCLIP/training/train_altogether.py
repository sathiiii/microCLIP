...
from MetaCLIP.open_clip.tokenizer import tokenize as clip_tokenizer
from MetaCLIP.training.distributed import is_master, world_info_from_env
from MetaCLIP.training.zero_shot import zero_shot_eval
from MetaCLIP.training.precision import get_autocast
from MetaCLIP.training.train import to_device, images_to_device, backward, AverageMeter
from MetaCLIP.training.checkpoint import unwrap_model, save_checkpoint, agg_positions, collect_positions
...
def create_captioner(args, model, device):
    ...
    for model_code in os.listdir(f"src/open_clip"):
        if model_code.endswith(".py") and "model" in model_code:
            module = importlib.import_module("MetaCLIPopen_clip." + model_code[:-len(".py")])
