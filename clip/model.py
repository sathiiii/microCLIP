    ...
    def __init__(self, inplanes: int, planes: int, stride: int = 1) -> None:
        ...
        if stride > 1 or inplanes != planes * self.expansion:
            self.downsample = nn.Sequential(OrderedDict([
                ...
                ("conv", nn.Conv2d(inplanes, planes * self.expansion, kernel_size=1, bias=False)),
...
class AttentionPool2d(nn.Module):
    def __init__(self, spatial_dim: int, embed_dim: int, num_heads: int, output_dim: int = None, return_local_features: bool = True) -> None:
        super().__init__()
...
class ModifiedResNet(nn.Module):
    """
    A ResNet class similar to torchvision's but with the following modifications:
    - Uses a 3-layer stem instead of a single convolution, with an average pool instead of max pool.
    - Applies antialiasing strided convolutions by prepending an avgpool to convolutions with stride > 1.
    - Replaces the final pooling layer with a QKV attention mechanism instead of an average pool.
    """
    def __init__(self, layers: list, output_dim: int, heads: int, input_resolution: int = 224, width: int = 64, return_local_features: bool = True) -> None:
        super().__init__()
        ...
        self.relu = nn.ReLU(inplace=True)
        # Residual layers
        self._inplanes = width  # Mutable variable used during layer construction
    ...
    def _make_layer(self, planes, blocks, stride=1):
        # This function should be defined to create the residual layers
        raise NotImplementedError("Define `_make_layer` to create ResNet blocks.")
...
class VisionTransformer(nn.Module):
    ...
    def forward_after_patch_embeddings(self, x: torch.Tensor):
        ...
        if self.proj is not None:
            x = F.normalize(x @ self.proj, dim=-1)
        return x

    def forward(self, x, return_cls_token = False, return_patch_tokens=False):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
